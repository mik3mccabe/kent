...
def normalize_search(data):
    # Frinkiac deployments have returned either a list or an object containing Simpsons results.
    rows=data if isinstance(data,list) else data.get('Results', data.get('results', []))
    out=[]
    for r in rows:
        ep=r.get('Episode') or r.get('episode') or {}
        episode=ep.get('Key') or ep.get('key') or r.get('EpisodeKey') or r.get('episodeKey') or ''
        ts=r.get('Timestamp') or r.get('timestamp') or 0
        text=r.get('Text') or r.get('text') or r.get('Subtitle') or ''
        if episode and ts is not None:
            out.append({'episode':episode,'timestamp':int(ts),'text':text})
    return out
